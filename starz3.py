...
def solvez3(regions):
    ...
    cells_c  = [ And(0 <= X[i][j], X[i][j] <= 1) for i in range(10) for j in range(10)]
    # each row sums to 2
    rows_c   = [And(sum(X[i]) == 2) for i in range(10)]
    # each column sums to 2
    cols_c   = [ And(sum([ X[i][j] for i in range(10) ]) == 2) for j in range(10)]
    # each region sums to 2
    regions_c = [And(sum(X[row][col] for (col,row) in region) == 2) for region in regions]
    # no 1s are adjacent
    adj_c = []
    ...
    if s.check() == sat:
        m = s.model()
        r = [ [ m.evaluate(X[i][j]) for j in range(10) ] 
              for i in range(10) ]
        print_matrix(r)
        return r
    else:
